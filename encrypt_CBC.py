...
def generate_dh_keys():
    private_key = getRandomRange(1, p - 1)  # Закрытый ключ
    public_key = pow(g, private_key, p)  # Открытый ключ
...
def encrypt_CBC(message):
    ...
    init = str(os.urandom(16))
    init = base64.b64encode(bytes(init, 'utf-8'))
    init = bytesToBin(init)[:4 * w]

    while len(init) % (4 * w) != 0:
        init = "0" + init

    synchro_package = init
    encoded_message_bit = ""

    for i in range(0, len(message_bit), 4 * w):
        temp = message_bit[i: i + 4 * w]
        temp = bin_expansion(bin(XOR(int("0b" + temp, 2), int("0b" + synchro_package, 2))), 4 * w)[2:]
    ...
    return encoded_message_bit, shared_secret, init
