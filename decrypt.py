...
def decrypt(encoded_message_bit, shared_secret):
    w = 32
    r = 20
    ...
    for i in range(0, len(encoded_message_bit[0]), 4 * w):
        A = int('0b' + encoded_message_bit[0][i:i + w], 2)
        B = int('0b' + encoded_message_bit[0][i + w:i + 2 * w], 2)
        C = int('0b' + encoded_message_bit[0][i + 2 * w:i + 3 * w], 2)
        D = int('0b' + encoded_message_bit[0][i + 3 * w:i + 4 * w], 2)
    ...
    decoded_message = base64.b64decode(binToBytes(decoded_message_bit)).decode('utf-8')
